...
电商公司用户访问时间数据。某个用户连续的访问记录如果时间间隔小于 60 秒，则分为同一个组。是对记录进行分组，而不是对用户。
...
我的理解：大局观角度分析问题。时间排序之后取差值。

按照用户分组之后的数据，计算第一行到当前行大于2天的总条数。这意思是：因为间隔一天(差值=2)也算连续，只有大于2天才不算连接。如果在某一天登录时，已经间隔了3天，后面又连续登录，那么这一天相当于新的统计起点，所以要能够标记出这个点。就使用：第一行到当前行大于2天的总条数。后面继续连续登录之后，后面的这个值就登录当前的值。

根据id和 flag分组。组内的数据都是满足要求的连续登录，所以只需要最大时间-最小时间+1。
...
思路：按照brand分组、起始时间排序之后，截止到当前行，将之前的最大结束日期移下来。
...
比较起始日期和之前的最大结束日期，加一是因为不可能是上个结束时间，而是结束时间的第二天。
...
截止到当前时刻，求和。相当于流式处理：上线个加一，离线一个减一。
